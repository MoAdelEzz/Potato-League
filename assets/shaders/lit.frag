...
void main() {
    ...
    for(int lightIndex = 0; lightIndex < min(MAX_LIGHTS, lightCount); lightIndex++){
        ...
        vec3 diffuse = light.diffuseColor * mat.diffuse * max(0.0, dot(fs_in.normal, normalize(lightDir)));
        ...
        if (mat.illumModel == 3 && false) {
            ...
            specular = light.specularColor * mat.specular * pow(max(0.0, dot(normalize(reflectDir), fs_in.viewDir)), mat.SpecularExponent);
        }
